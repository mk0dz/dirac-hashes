#!/usr/bin/env python3
"""
Hash Function Benchmarking and Security Analysis

This script benchmarks our quantum-inspired hash functions against SHA-256
and performs security analysis on the hash outputs.
"""
...
def analyze_distribution(hash_funcs: Dict[str, Callable], 
    ...
    for name, func in hash_funcs.items():
        byte_counts = [Counter() for _ in range(32)]  # Assuming 32-byte digest
        byte_values = []
        ...
        # Normalize entropy (max is 8 bits per byte)
        entropy = entropy / 8
...
def plot_speed_results(results: Dict[str, Dict[int, float]], sizes: List[int], 
                      title: str = "Hash Function Performance") -> None:
    """Plot the speed benchmark results."""
    plt.figure(figsize=(10, 6))
    
    for name, size_times in results.items():
        times = [size_times[size] * 1000 for size in sizes]  # Convert to milliseconds
...
def main():
    ...
    collision_results = collision_test(hash_funcs, samples=10000)
    
    # Plot results
    print("Plotting results...")
    plot_speed_results(speed_results, sizes)
